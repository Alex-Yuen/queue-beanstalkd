...
class Beanstalkd extends Job {
    private ?Banstalkd $banstalkd = null;
    ...
    public function __construct(App $app, Banstalkd $beanstalk, \Pheanstalk\Job $job, string $connection, string $queue) {
        ...
        $this->banstalkd = $beanstalk;
        $this->connection = $connection;
        $this->attempts = $this->payload()['attempts'] ?? 1;
    }

    public function payload(): array {
        return empty($this->parseRaw) ? $this->parseRaw = $this->banstalkd->unpack($this->job->getData()) : $this->parseRaw;
    }
    ...
    public function release($delay = 0) {
        ...
        $this->delete();
        $this->banstalkd->release($this->queue, $this->payload(), $delay, ++$this->attempts);
    }
